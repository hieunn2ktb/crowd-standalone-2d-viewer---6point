    ...
    if (_common.isNotEmpty(imageData.objectList)) {
        imageData.objectList.forEach(function (object) {
            ...
            if (object.objectType == page.constants.drawType.rectangle) {
                ...
                minXY = page.fn.render.fn6PointCube(object);
            } else {
            ...
            if (_common.isNotEmpty(object.groupId)) {
                ...
                i.setAttribute("href", "/image/tool/icon_qr.png");
                ...
                //<image href="/global/image/tool/icon_qr.png" height="200" width="200"></image>
                tagIndex++;
            }
        });
    }
...
page.fn.getFileStatusName = function (status, statusResult, statusInspect) {
    ...
    switch (status) {
        case "open"       :
            name = page.message.status.open;
            break;
        case "assigned"   :
            name = page.message.status.assigned;
            break;
        case "worked"     :
            name = page.message.status.worked;
            break;
        case "rvassigned" :
            name = page.message.status.rvAssigned;
            break;
        case "reviewed"   :
...
page.fn.getStatusName = function (o) {
    ...
    switch (o.status) {
        case 'open'         :
            statusName = "[Open]작업대기";
            break;
        case 'assigned'     :
            statusName = "[Assigned]작업중";
            break;
        case 'worked'       :
            statusName = "[Worked]작업완료";
            break;
        case 'rvassigned'   :
            statusName = "[Review Assigned]검수중";
            break;
        case 'reviewed' :
...
// #region 6pointcube
page.fn.render.fn6PointCube = function (object) {

    log.info(object, "page.fn.render.fn6PointCube");
    let minXY = {x: 0, y: 0};
    try {
        ...
        if (!Array.isArray(loc) || loc.length < 2) {
            log.warn(object, "Invalid location data");
            return minXY;
        }

        // Tính bounding box từ 2 điểm đầu tiên
        const [pt1, pt2] = loc;
        ...
        // Tạo và thêm <rect> vào SVG
        const svg = document.getElementById("svg_" + object.workTicketId);
        const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
        g.setAttribute('objectId', object.objectId)
        g.setAttribute('data-objectType', object.objectType)
        svg.appendChild(g);
        ...
        rect.setAttribute("class", "draw-object object-body CLASS_" + object.classId);
        rect.setAttribute("id", object.objectId);
        rect.setAttribute("objectId", object.objectId);
        rect.setAttribute("title", page.fn.render.getTitle?.(object) || "");
        rect.setAttribute("data-workTicketId", object.workTicketId);
        rect.setAttribute("data-objectId", object.objectId);
        ...
        // Nếu có đủ 6 điểm và là object đã lưu
        if(loc.length === 8){

            const [p1, p2, p3, p4, p5, p6, p7, p8] = loc;
            const allPointListOfTheBox = {
                p1: {x: p1[0], y: p1[1], v:p1[2], id: p1[3]},
                p2: {x: p2[0], y: p2[1], v:p2[2], id: p2[3]},
                p3: {x: p3[0], y: p3[1], v:p3[2], id: p3[3]},
                p4: {x: p4[0], y: p4[1], v:p4[2], id: p4[3]},
                p5: {x: p5[0], y: p5[1], v:p5[2], id: p5[3]},
                p6: {x: p6[0], y: p6[1], v:p6[2], id: p6[3]},
                p7: {x: p7[0], y: p7[1], v:p7[2], id: p5[3]},
                p8: {x: p8[0], y: p8[1], v:p8[2], id: p6[3]},
            ...
            const options = {
                front: "blue", rear: "red", side: "green",
                p1P2Color: "#000000", p3Color: "#yellow",
                p4Color: "#blue", p5Color: "#red", p6Color: "#green", p7Color: "#000000", p8Color: "#000000"
            };

            object.location = loc
            updateShapeColors(svg, $(g), object, options);
        }



    } catch (E) {
        log.debug(E, "page.fn.render.fn6PointCube");
    }
 ...
 */
function updateShapeColors(svgEl, g, obj, options) {
    // Chuẩn hóa SVG element nếu là jQuery
    const svg = svgEl instanceof jQuery ? svgEl[0] : svgEl;
    const $svg = $(svg);

    const pointList = obj.allPointListOfTheBox;
    if (!pointList) return;

    const [x1, y1] = obj.location[0];
    const [x2, y2] = obj.location[1];
    const [x4] = obj.location[3];
    const centerX = x1 + (x2 - x1) / 2;

    const isRightSide = x4 >= centerX;
    const pointsFrontBox = isRightSide
        ? [pointList.p1, pointList.p3, pointList.p6, pointList.p8]
        : [pointList.p3, pointList.p7, pointList.p2, pointList.p6];

    // Xóa polygon cũ và làm mờ rect
    $svg.find("polygon[data-group='pseudo-6pointCube']").remove();
    $svg.find(`rect[data-group='6pointCube'][objectId='${obj.objectId}']`)
        .attr({
    ...
    // Vẽ lại các polygon đại diện cho mặt trước & mặt bên
    drawPolygonIfMissing(g, obj, "quad1-6pointCube", pointsFrontBox,
        obj.direction6PointCube === 'front' ? options.front : options.rear, 0, true);

    drawPolygonIfMissing(g, obj, "quad2-6pointCube",
        [pointList.p3, pointList.p4, pointList.p5, pointList.p6],
        options.side, 0, false);

    // Đảm bảo <rect> luôn ở cuối trong <g>
    const $g = $(`g[objectId='${obj.objectId}'][data-objectType='${obj.objectType}']`);
    $g.find(`rect[data-group='6pointCube'][objectId='${obj.objectId}']`).appendTo($g);
}
...
function drawPolygonIfMissing(svgEl, obj, groupName, points, color, opacity = 0, isSolidLine = true) {
    ...
    const polygon = $(document.createElementNS("http://www.w3.org/2000/svg", "polygon"))
        ...
        .attr("class", "draw-object CLASS_" + obj.classId);
